...
apt install -y php$PHP_V-curl php$PHP_V-cgi php$PHP_V-bcmath	> $LOG_FILE 2>&1
apt install -y php$PHP_V-zip php$PHP_V-mbstring php$PHP_V-xml php$PHP_V-xmlrpc	> $LOG_FILE 2>&1

echo -en "\033[1;33m Install PHP DB \033[0m\n"
apt install -y php$PHP_V-pdo php$PHP_V-intl > $LOG_FILE 2>&1

echo -en "\033[1;33m Install PHP Graphics \033[0m\n"
apt install -y php$PHP_V-gd		> $LOG_FILE 2>&1
apt install -y php$PHP_V-imagick	> $LOG_FILE 2>&1

echo -en "\033[1;33m Install PHP Dev \033[0m\n"
apt install -y php$PHP_V-dev	> $LOG_FILE 2>&1
...
for file in $(ls -a) ; do 
	if [[ $file =~ "mysql" ]]; then
		...
		apt install -y php$PHP_V-mysql	> $LOG_FILE 2>&1
	fi
...
apt install -y php$PHP_V apache2- apache2-bin-	> $LOG_FILE 2>&1
apt install -y php$PHP_V-fpm	> $LOG_FILE 2>&1
